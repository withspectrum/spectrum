...
export const TitleBar = styled(FlexRow)`
  ...
  padding: ${isDesktopApp() ? '16px 8px 0' : '0 8px'};
  background-color: ${({ theme }) => theme.bg.reverse};
  color: ${({ theme }) => theme.text.reverse};
  min-height: ${isDesktopApp() ? '64px' : '48px'};
  height: ${isDesktopApp() ? '64px' : '48px'};
  max-height: ${isDesktopApp() ? '64px' : '48px'};
  order: 0;
  flex: 0 0 ${isDesktopApp() ? '64px' : '48px'};
