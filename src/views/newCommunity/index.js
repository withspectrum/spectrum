...
} from '../communitySettings/components/importSlack';
import {
  EmailInvitesWithoutCard,
} from '../communitySettings/components/emailInvites';
...
class NewCommunity extends Component {
  ...
  title = () => {
    ...
    switch (activeStep) {
      ...
      case 2: {
        return `Invite people${community ? ` to the ${community.name} community` : ' to your community'}`;
      }
      case 3: {
        return 'Done!';
      }
  ...
  description = () => {
    ...
    switch (activeStep) {
      ...
      case 2: {
        return `Kickstart ${community ? `the ${community.name} community` : 'your community'} by inviting an existing Slack team or by inviting a handful of folks directly by email. You'll be able to invite more people at any point in the future, too, if you're not quite ready.`;
      }
      case 3: {
  ...
  render() {
    const { activeStep, community, existingId } = this.state;
      ...
      <AppViewWrapper>
        ...
        <Column type="primary">
          <Container bg={activeStep === 3 ? 'onboarding' : null} repeat>
            ...
            <Description centered={activeStep === 3}>{description}</Description>
