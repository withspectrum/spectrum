...
const SpectrumButton = styled.button`
  ...
  props.subtle || iconOnly(props) ? 'transparent' : buttonColor(props)};
  background-image: ${props =>
  props.subtle || props.disabled || iconOnly(props)
    ? 'none'
    : Gradient(props.theme.brand.alt, props.theme.brand.default)};
  border: 2px solid ${props =>
  props.subtle ? props.theme.bg.wash : 'transparent'};
  border-radius: 8px;
  padding: 8px 16px;
  font-size: 14px;
  font-weight: ${props => props.subtle ? '500' : '600'};
  color: ${props =>
  props.subtle ? buttonColor(props) : props.theme.text.reverse};
  ...
  &:hover{
    ...
    ${props =>
  props.subtle ? `background-color: ${props.theme.bg.default};` : ''};
    transition: ${Transition.hover.on};
    box-shadow: ${props =>
  props.disabled || iconOnly(props) ? 'none' : Shadow.high};
