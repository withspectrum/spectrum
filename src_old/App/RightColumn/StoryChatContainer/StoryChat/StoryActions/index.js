...
class StoryActions extends Component {
  ...
  render() {
    ...
    const canDelete =
      story.creator.uid === currentUser.uid || communityRole === 'owner';
    const canFreeze = communityRole === 'owner';
    const shareUrl = `https://spectrum.chat/${community.slug}/~${frequency.slug}/${story.id}`;
    return (
      <ActionBarContainer>
        <a
          href={`https://twitter.com/intent/tweet/?text=${encodeURIComponent(story.content.title.substr(0, 85))}&amp;url=${shareUrl}`}
          target="_blank"
